            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-8">
                ...
                <div class="flex items-start justify-between mb-6 pb-5 border-b border-gray-100">
                    <div>
                        ...
                        <p class="text-sm text-gray-400 mt-0.5">Project Description: {{ $project->description ?: 'No description provided.' }}
                        </p>
                    </div>
                ...
                @else
                    <div id="issuesList" class="flex flex-col gap-2">
                        @foreach ($project->issues as $issue)
                            <div class="issue-row flex items-center justify-between px-4 py-3 border border-gray-200 rounded-xl hover:border-gray-300 transition"
                                data-status="{{ $issue->status }}" data-priority="{{ $issue->priority }}"
                                data-tags="{{ $issue->tags->pluck('id')->join(',') }}">

                                <div class="flex flex-col gap-1 min-w-0 pr-6 pl-2">
                                    <span
                                        class="text-sm font-semibold text-gray-900 truncate">{{ $issue->title }}</span>
                    ...
                    <p id="noResults" class="hidden text-sm text-gray-400 text-center py-8">No issues match your
                        filters.</p>
                @endif
